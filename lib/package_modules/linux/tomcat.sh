...
ensureSymLinksExist () {
    ...
    then
        ...
        ln -s "$ENVIRONMENT_ROOT_DIR"/bin/.linux/.tomcat/"$versionName"/bin/startup.sh "$ENVIRONMENT_ROOT_DIR"/bin/tomcat_start
    else
        ln -s "$ENVIRONMENT_ROOT_DIR"/bin/.linux/.tomcat/"$versionName"/bin/startup.sh "$ENVIRONMENT_ROOT_DIR"/bin/tomcat_start
    fi
    ...
    then
        ...
        ln -s "$ENVIRONMENT_ROOT_DIR"/bin/.linux/.tomcat/"$versionName"/bin/shutdown.sh "$ENVIRONMENT_ROOT_DIR"/bin/tomcat_stop
    else
        ln -s "$ENVIRONMENT_ROOT_DIR"/bin/.linux/.tomcat/"$versionName"/bin/shutdown.sh "$ENVIRONMENT_ROOT_DIR"/bin/tomcat_stop
    fi
...
then
    ...
    case $environmentVariableSetup in
        ...
        "-environmentSetup")
            ...
            echo "CATALINA_HOME=$ENVIRONMENT_ROOT_DIR/bin/.linux/.tomcat/$latestVersionNameFromWeb :: CATALINA_BASE=$ENVIRONMENT_ROOT_DIR/bin/.linux/.tomcat/$latestVersionNameFromWeb :: CATALINA_TMPDIR=$ENVIRONMENT_ROOT_DIR/bin/.linux/.tomcat/$latestVersionNameFromWeb/temp"
        ;;
        *)
